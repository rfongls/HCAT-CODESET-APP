...
def test_close_buttons_render(tmp_path, monkeypatch):
    ...
    assert '<button id="overlayClose" type="button" class="overlay-close" aria-label="Close" title="Close">&times;</button>' in text
    assert '<button id="themeClose" type="button" class="overlay-close" aria-label="Close" title="Close">&times;</button>' in text
